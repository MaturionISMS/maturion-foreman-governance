 ...
 * Original error: Build error detected: > Build error occurred
 * Source: build.log:12
 */
...
describe('Regression: BuildError', () => {
  it('should-not-have-build-in-build-log-12', () => {
    ...
    const errorPattern = /Build.*error.*detected.*Build.*error/i;
    
    assert.ok(
      !errorPattern.test(buildLog),
      `Build log should not contain BuildError: Build error detected: > Build error occurred`
    );
  });
});
